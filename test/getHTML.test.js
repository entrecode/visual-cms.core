...
line2<br>`);
  ...
  it('with id but no array in content', (done) => {
    ...
    expect(y).to.match(/^<div data-ec-id="[a-f0-9-]+"><p data-ec-id="[a-f0-9-]+"><span data-ec-id="[a-f0-9-]+">parent\.\.\.\?<\/span><\/p><p data-ec-id="[a-f0-9-]+" class="class1 class2"><span data-ec-id="[a-f0-9-]+">child<\/span><strong data-ec-id="[a-f0-9-]+" class="extrastrong">grandchild<\/strong><span data-ec-id="[a-f0-9-]+">toy\n<\/span><\/p><\/div>$/)
    done();
  });
  it('list', (done) => {
    const text = new core.elements.Text('test');
